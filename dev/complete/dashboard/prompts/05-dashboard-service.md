# Feature: dashboard-service - Dashboard Data Service

## Context

Types are defined in `viewer/types.ts`. Configuration paths are in `viewer/config.ts`. The existing `LogFileWatcher` in `viewer/watcher.ts` provides `listSessions()` to discover session log files.

## Objective

Create a new `DashboardService` class that gathers all dashboard data from various sources.
...
- Create new file `viewer/dashboard.ts`
- Use existing `LogFileWatcher.listSessions()` for session discovery
- Parse YAML frontmatter from markdown files
- Handle missing files gracefully

## Files to Create

- `.claude/hooks/viewer/dashboard.ts` - New dashboard data service
...
- [ ] `getData()` returns complete `DashboardData` object
- [ ] `getSessions()` returns sessions with computed status
- [ ] Status is "ended" if SessionEnd exists without restart
- [ ] Status is "inactive" if last activity > 60s ago
- [ ] Status is "active" for recent activity
- [ ] `getGlobalStats()` reads from `~/.claude/stats-cache.json`
- [ ] `getCommands()` parses `.claude/commands/*.md`
- [ ] `getSkills()` parses `.claude/skills/*.md`
- [ ] `getHooks()` reads from `settings.json`
- [ ] `getMcpServers()` reads `enabledMcpjsonServers`
- [ ] All methods handle missing files gracefully
...
git commit -m "feat(dashboard): add dashboard data service"
```

## Next

Proceed to: `prompts/06-api-endpoint.md`
